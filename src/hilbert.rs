...
mod tests {
    ...
    fn test_hilbert_quadrant_mapping() {
        ...
        assert!(
            x_00 < mid && y_00 < mid,
            "0.0.0.0/2 should be in top-left quadrant, got ({}, {})",
            x_00,
            y_00
        );
        ...
        assert!(
            x_64 < mid && y_64 >= mid,
            "64.0.0.0/2 should be in lower-left quadrant, got ({}, {})",
            x_64,
            y_64
        );
        ...
        assert!(
            x_128 >= mid && y_128 >= mid,
            "128.0.0.0/2 should be in lower-right quadrant, got ({}, {})",
            x_128,
            y_128
        );
        ...
        assert!(
            x_192 >= mid && y_192 < mid,
            "192.0.0.0/2 should be in top-right quadrant, got ({}, {})",
            x_192,
            y_192
        );
    }
    ...
    fn test_240_0_0_0_slash_4() {
        ...
        assert!(
            x_240 >= mid,
            "240.0.0.0/4 should be in right half, got x={}",
            x_240
        );
        assert!(
            y_240 < quarter,
            "240.0.0.0/4 should be in upper quarter, got y={}",
            y_240
        );
    }
    ...
    fn test_hilbert_order_consistency() {
        ...
        for order in 1..=12 {
            ...
            assert!(
                x <= max_coord,
                "x coordinate {} exceeds max {} for order {}",
                x,
                max_coord,
                order
            );
            assert!(
                y <= max_coord,
                "y coordinate {} exceeds max {} for order {}",
                y,
                max_coord,
                order
            );
    ...
    fn test_specific_ip_mappings() {
        ...
        for (ip, ip_str, expected_quadrant) in test_cases {
            ...
            assert_eq!(
                ...
                "{} should be in {} quadrant, got {}",
                ip_str, expected_quadrant, actual_quadrant
            );
        }
    }
}
